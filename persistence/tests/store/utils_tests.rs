...
use persistence::{Store, WorkflowDefinition, WorkflowExecution, UserPrompt, AppSettings, AuditEvent, AuditStatus, WorkflowStatus, TaskExecution, TaskStatus};
use chrono::Utc;
...
async fn test_clear_all_data_with_data() {
    ...
    store.save_task_execution(TaskExecution {
        id: "task-1".to_string(),
        workflow_id: "exec-1".to_string(),
        name: "Test Task".to_string(),
        status: TaskExecutionStatus::Complete,
    ...
    // Settings should return to default after clear
    let loaded_settings = store.load_settings().await.unwrap();
    assert_eq!(loaded_settings.theme, persistence::Theme::System);
    assert!(loaded_settings.auto_save);
    assert!(loaded_settings.notifications);
    assert!(loaded_settings.default_workflow.is_none());
}
