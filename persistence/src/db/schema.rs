...
fn create_indexes(conn: &Connection) -> Result<(), PersistenceError> {
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_workflows_name ON workflows(json_extract(data, '$.name'))",
        [],
    )?;
    conn.execute(
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_wf_exec_instance ON workflow_executions(instance_id)",
        [],
    )?;
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_task_exec_instance ON task_executions(instance_id)",
        [],
    )?;
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_user_prompts_instance ON user_prompts(instance_id)",
        [],
    )?;
    ...
    // AI prompts indexes
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_ai_prompts_instance ON ai_prompts(instance_id)",
        [],
    )?;
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_settings_created ON settings(created_at)",
        [],
    )?;
