...
impl WorkflowStore {
    ...
    pub async fn save(&self, workflow: &Workflow) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(workflow).map_err(|e| {
            PersistenceError::Serialization(format!("Failed to serialize workflow: {}", e))
        })?;
    ...
    pub async fn get(&self, id: &str) -> Result<Option<Workflow>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let workflow: Workflow = serde_json::from_str(&data).map_err(|e| {
                PersistenceError::Serialization(format!("Failed to deserialize workflow: {}", e))
            })?;
    ...
    pub async fn list(
        ...
        limit: usize,
        offset: usize,
    ) -> Result<Vec<Workflow>, PersistenceError> {
        debug!("Listing workflows (limit: {}, offset: {})", limit, offset);

        let conn = self.pool.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT data FROM workflows ORDER BY created_at DESC LIMIT ?1 OFFSET ?2")?;
    ...
    pub async fn list_by_instance(
        ...
        debug!(
            "Listing workflows for instance: {} (limit: {}, offset: {})",
    ...
    pub async fn search_by_name(
        ...
        debug!(
            "Found {} workflows matching pattern '{}'",
            workflows.len(),
            name_pattern
        );
        Ok(workflows)
