...
impl SettingsStore {
    ...
    pub async fn save(&self, setting: &Setting) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(setting).map_err(|e| {
            PersistenceError::Serialization(format!("Failed to serialize setting: {}", e))
        })?;
    ...
    pub async fn get(&self, key: &str) -> Result<Option<Setting>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let setting: Setting = serde_json::from_str(&data).map_err(|e| {
                PersistenceError::Serialization(format!("Failed to deserialize setting: {}", e))
            })?;
    ...
    pub async fn list_by_instance(
        &self,
        instance_id: &str,
    ) -> Result<Vec<Setting>, PersistenceError> {
        debug!("Listing settings for instance: {}", instance_id);

        let conn = self.pool.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT data FROM settings WHERE instance_id = ?1 ORDER BY key ASC")?;
