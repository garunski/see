...
impl TaskExecutionStore {
    ...
    pub async fn save(&self, task: &TaskExecution) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(task)
            .map_err(|e| PersistenceError::Serialization(format!("Failed to serialize task execution: {}", e)))?;
    ...
    pub async fn get(&self, id: &str) -> Result<Option<TaskExecution>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let task: TaskExecution = serde_json::from_str(&data)
                .map_err(|e| PersistenceError::Serialization(format!("Failed to deserialize task execution: {}", e)))?;
    ...
    pub async fn list(&self, limit: usize, offset: usize) -> Result<Vec<TaskExecution>, PersistenceError> {
        debug!("Listing task executions (limit: {}, offset: {})", limit, offset);
    ...
    pub async fn list_by_workflow_execution(&self, workflow_execution_id: &str) -> Result<Vec<TaskExecution>, PersistenceError> {
        debug!("Listing task executions for workflow execution: {}", workflow_execution_id);
        ...
        debug!("Found {} tasks for workflow execution {}", tasks.len(), workflow_execution_id);
        Ok(tasks)
    }
    
    /// List task executions by status
    pub async fn list_by_status(&self, status: &str) -> Result<Vec<TaskExecution>, PersistenceError> {
    ...
    pub async fn list_by_instance(&self, instance_id: &str, limit: usize, offset: usize) -> Result<Vec<TaskExecution>, PersistenceError> {
        debug!("Listing task executions for instance: {} (limit: {}, offset: {})", instance_id, limit, offset);
    ...
    pub async fn count_by_status(&self, status: &str) -> Result<usize, PersistenceError> {
        ...
        let conn = self.pool.get_connection()?;
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM task_executions WHERE json_extract(data, '$.status') = ?1")?;
    ...
    pub async fn count_by_workflow_execution(&self, workflow_execution_id: &str) -> Result<usize, PersistenceError> {
        debug!("Counting task executions for workflow execution: {}", workflow_execution_id);
        
        let conn = self.pool.get_connection()?;
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM task_executions WHERE workflow_execution_id = ?1")?;
        let count: i64 = stmt.query_row([workflow_execution_id], |row| row.get(0))?;
        
        debug!("Task executions for workflow execution {}: {}", workflow_execution_id, count);
        Ok(count as usize)
    }
}
