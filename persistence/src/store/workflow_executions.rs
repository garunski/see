...
impl WorkflowExecutionStore {
    ...
    pub async fn save(&self, execution: &WorkflowExecution) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(execution).map_err(|e| {
    ...
    pub async fn get(&self, id: &str) -> Result<Option<WorkflowExecution>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let execution: WorkflowExecution = serde_json::from_str(&data).map_err(|e| {
                PersistenceError::Serialization(format!(
    ...
    pub async fn list(
        ...
        debug!(
            "Listing workflow executions (limit: {}, offset: {})",
            limit, offset
        );
    ...
    pub async fn list_by_instance(
        ...
        debug!(
            "Listing workflow executions for instance: {} (limit: {}, offset: {})",
    ...
    pub async fn count_by_status(&self, status: &str) -> Result<usize, PersistenceError> {
        ...
        let conn = self.pool.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT COUNT(*) FROM workflow_executions WHERE json_extract(data, '$.status') = ?1",
        )?;
