...
impl UserPromptStore {
    ...
    pub async fn save(&self, prompt: &UserPrompt) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(prompt).map_err(|e| {
            PersistenceError::Serialization(format!("Failed to serialize user prompt: {}", e))
        })?;
    ...
    pub async fn get(&self, id: &str) -> Result<Option<UserPrompt>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let prompt: UserPrompt = serde_json::from_str(&data).map_err(|e| {
                PersistenceError::Serialization(format!("Failed to deserialize user prompt: {}", e))
            })?;
    ...
    pub async fn list_by_instance(
        &self,
        instance_id: &str,
    ) -> Result<Vec<UserPrompt>, PersistenceError> {
        ...
        debug!(
            "Found {} user prompts for instance {}",
            prompts.len(),
            instance_id
        );
        Ok(prompts)
    ...
    pub async fn search_by_name(
        ...
        debug!(
            "Found {} user prompts matching pattern '{}'",
            prompts.len(),
            name_pattern
        );
        Ok(prompts)
