...
impl AiPromptStore {
    ...
    pub async fn save(&self, prompt: &AiPrompt) -> Result<(), PersistenceError> {
        ...
        let data = serde_json::to_string(prompt)
            .map_err(|e| PersistenceError::Serialization(format!("Failed to serialize AI prompt: {}", e)))?;
    ...
    pub async fn get(&self, id: &str) -> Result<Option<AiPrompt>, PersistenceError> {
        ...
        if let Some(row) = rows.next() {
            let data = row?;
            let prompt: AiPrompt = serde_json::from_str(&data)
                .map_err(|e| PersistenceError::Serialization(format!("Failed to deserialize AI prompt: {}", e)))?;
    ...
    pub async fn list_by_instance(&self, instance_id: &str) -> Result<Vec<AiPrompt>, PersistenceError> {
        ...
        debug!("Found {} AI prompts for instance {}", prompts.len(), instance_id);
        Ok(prompts)
    }
    
    /// Search AI prompts by name
    pub async fn search_by_name(&self, name_pattern: &str) -> Result<Vec<AiPrompt>, PersistenceError> {
        ...
        debug!("Found {} AI prompts matching pattern '{}'", prompts.len(), name_pattern);
        Ok(prompts)
    }
