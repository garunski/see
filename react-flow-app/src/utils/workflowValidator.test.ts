...
describe('workflowValidator', () => {
  ...
  const createCliTask = (id: string, name: string, command: string = ''): WorkflowTask => ({
    id,
    name,
    function: {
      name: 'cli_command',
  ...
  const createCursorTask = (id: string, name: string, prompt: string = ''): WorkflowTask => ({
    id,
    name,
    function: {
      name: 'cursor_agent',
  ...
  const createUserInputTask = (id: string, name: string, prompt: string = '', input_type: string = 'string'): WorkflowTask => ({
    id,
    name,
    function: {
      name: 'user_input',
  ...
  describe('Structural Validation', () => {
    ...
    it('should require workflow name', () => {
      ...
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
      
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message === 'Workflow must have a name')).toBe(true);
    });
    ...
    it('should require workflow ID', () => {
      ...
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message === 'Workflow must have an ID')).toBe(true);
    });
    ...
    it('should detect disconnected nodes', () => {
      ...
      const task = createCliTask('task1', 'Task 1', 'echo');
      const nodes = [createNode(task)];
      const edges: Edge[] = []; // No connection to START
      
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message.includes('not connected to the workflow'))).toBe(true);
    });
  ...
  describe('CLI Command Validation', () => {
    it('should require command field', () => {
      const workflow = createMockWorkflow();
      const task = createCliTask('task1', 'CLI Task', ''); // Empty command
      const nodes = [createNode(task)];
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
      
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message.includes('command') && e.message.includes('required'))).toBe(true);
    });
  ...
  describe('Cursor Agent Validation', () => {
    it('should require prompt field', () => {
      const workflow = createMockWorkflow();
      const task = createCursorTask('task1', 'Agent Task', ''); // Empty prompt
      const nodes = [createNode(task)];
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
      
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message.includes('prompt') && e.message.includes('required'))).toBe(true);
    });
  ...
  describe('User Input Validation', () => {
    it('should require prompt field', () => {
      const workflow = createMockWorkflow();
      const task = createUserInputTask('task1', 'Input Task', '', 'string'); // Empty prompt
      const nodes = [createNode(task)];
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
    ...
    it('should require input_type field', () => {
      const workflow = createMockWorkflow();
      const task = createUserInputTask('task1', 'Input Task', 'Enter value', ''); // Empty input_type
      const nodes = [createNode(task)];
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
      
      const errors = validateWorkflow(nodes, edges, workflow);
      expect(errors.some(e => e.message.includes('input_type') && e.message.includes('required'))).toBe(true);
    });

    it('should pass with valid user input', () => {
      const workflow = createMockWorkflow();
      const task = createUserInputTask('task1', 'Input Task', 'Enter name', 'string');
      const nodes = [createNode(task)];
      const edges: Edge[] = [{ id: 'e1', source: START_NODE_ID, target: 'task1' }];
