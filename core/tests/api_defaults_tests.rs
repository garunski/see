...
fn test_default_workflows() {
    ...
    for workflow in &defaults {
        assert!(
            workflow.is_default,
            "Default workflow should have is_default = true"
        );
        assert!(
            !workflow.content.is_empty(),
            "Default workflow should have content"
        );
    }
...
fn test_default_workflows_content_valid() {
    ...
    for workflow in &defaults {
        ...
        assert!(
            parse_result.is_ok(),
            "Default workflow content should be valid JSON: {}",
            workflow.id
        );
        ...
        assert!(
            ...
            "Default workflow should have name field: {}",
            workflow.id
        );
        assert!(
            json.get("tasks").is_some(),
            "Default workflow should have tasks field: {}",
            workflow.id
        );
    }
}
...
fn test_default_workflows_unique_ids() {
    ...
    for workflow in &defaults {
        assert!(
            ids.insert(&workflow.id),
            "Default workflow IDs should be unique: {}",
            workflow.id
        );
    }
}
