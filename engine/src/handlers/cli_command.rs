...
impl super::TaskHandler for CliCommandHandler {
    ...
    async fn execute(
        ...
        context.log_task(task.id.clone(), format!("Executing CLI command: {} {:?}", command, args));
        ...
        if success {
            ...
            context.log_task(task.id.clone(), format!("Command failed with exit code: {:?}", output.status.code()));
        }
