use simple_workflow_app::db::{AuditStore, RedbAuditStore, WorkflowExecution, WorkflowExecutionSummary};
...
fn test_delete_workflow_execution() {
    run_test(|| {
        ...
        assert!(delete_result.is_ok(), "Should delete execution successfully");
        
        // Verify it's gone
        let retrieved_after = store.get_workflow_execution("test_delete");
        assert!(retrieved_after.is_err(), "Should not find deleted execution");
...
fn test_execution_summary_fields() {
    run_test(|| {
        ...
        let execution = create_test_execution("summary_test", "Summary Test Workflow");
