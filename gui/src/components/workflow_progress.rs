...
pub fn WorkflowProgress(
    ...
    let step_data: Vec<_> = tasks.iter().enumerate().map(|(step_idx, task)| {
        ...
        let status = if step_idx < current_step { if task.status == "failed" { "failed" } else { "complete" } } else if step_idx == current_step { "current" } else { "upcoming" };
                ...
                for (step_idx, task, is_last, step_number, status, button_class, line_class, number_class) in step_data {
                    li { class: if is_last { "relative" } else { "relative pr-8 sm:pr-20" },
                        ...
                        button { class: button_class, onclick: move |_| on_step_click.call(step_idx),
                            if status == "complete" && task.status != "failed" { span { "aria-hidden": "true", class: "size-5", "✓" } }
                            ...
                            if status != "complete" || task.status == "failed" { span { class: format!("absolute inset-0 flex items-center justify-center text-xs font-bold {}", number_class), {step_number.to_string()} } }
